...
inline LandmarkObs body2map(const LandmarkObs obs_body, const double x_p, const double y_p, const double theta_p){
  LandmarkObs obs_map;

  double ctheta = cos(theta_p);
  double stheta = sin(theta_p);

  obs_map.id = obs_body.id;
  obs_map.x = ( obs_body.x*ctheta - obs_body.y*stheta + x_p );
  obs_map.y = ( obs_body.x*stheta + obs_body.y*ctheta + y_p );

  return obs_map;
}


inline double probability2d(const LandmarkObs obs,
                            const std::vector<LandmarkObs> pred_landmarks,
                            const double std[]) {
  double prob = 1.0;

  // look for matching landmark
  //    (this would be much better using a std::map<> container...)
  int lm = 0;
  while (lm < pred_landmarks.size()) {

    if (pred_landmarks[lm].id == obs.id) {
      // Matching landmark found. Compute probability
      double power = -( 0.5*(obs.x-pred_landmarks[lm].x)*(obs.x-pred_landmarks[lm].x)/(std[0]*std[0])
                       +0.5*(obs.y-pred_landmarks[lm].y)*(obs.y-pred_landmarks[lm].y)/(std[1]*std[1]));
      prob = exp(power)/(2.0*M_PI*std[0]*std[1]);
      break;
    }
    lm++;
    // How to handle if no match found?
...
inline double * getError(double gt_x, double gt_y, double gt_theta, double pf_x, double pf_y, double pf_theta) {
  ...
  if (error[2] > M_PI) {
//    error[2] = 2.0 * M_PI - error[2]; // Is this in ERROR?
    error[2] -= 2.0 * M_PI; // limit to +/-PI instead of (0,2PI)
  }
  return error;
}
